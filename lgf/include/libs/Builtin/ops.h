...
namespace lgf
{
    ...
    public:
        ...
        {
            printer p;
            p << represent_output() << " = Declare " << output()->desc_represent();
            return p.dump();
        }
        virtual resultCode rewrite(painter p, node *op)
        {
            if (op->output()->get_user_size() == 0)
            {
                op->erase();
    ...
    public:
        ...
        static updateOp *build(node *lhs, node *rhs)
        {
            ...
            op->set_value_desc(rhs->output()->get_desc());
            op->accept(lhs, rhs);
            return op;
        }
        node *lhs() { return input(0)->get_defining_op(); }
        node *rhs() { return input(1)->get_defining_op(); }
        virtual std::string represent()
        {
            printer p;
            p << represent_output() << " = " << get_sid() << " : " << input(0)->represent() << " by " << input(1)->represent();
            return p.dump();
        }
    };
    ...
    public:
        ...
        virtual std::string represent()
        {
            printer p;
            p << represent_output() << " = "
              << "Constant: " << represent_output() << " = " << data->represent();
            return p.dump();
        }
